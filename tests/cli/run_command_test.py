...
def test_run_gerrit(mocker, tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=workspace.root):
        runner.invoke(gerrit_dev_tool, ["run"])

        subprocess.run.assert_any_call(["./bin/gerrit.sh", "run"], cwd=workspace.testsite, check=True)


def test_run_gerrit_from_any_directroy(mocker, tmp_path):
    mocker.patch("subprocess.run")
